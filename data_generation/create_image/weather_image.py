...
# Detailed prompt generation template for WEATHER edits in off-road scenarios
PROMPT_GENERATION_TEMPLATE = """
For the weather edit instruction: "{edit_instruction}", generate **two** detailed English image-generation prompts for off-road extreme weather scenarios:
1) a BEFORE-EDIT prompt that depicts the off-road scene before the weather change (original weather condition),
2) an AFTER-EDIT prompt that depicts the off-road scene after the weather change (target weather condition).

ATTENTION: 
** Focus on off-road terrain and extreme weather conditions **
** Weather changes MUST be SIGNIFICANT and OBVIOUS with dramatic visual differences **
** Include terrain details**

REQUIREMENTS (follow these exactly):
- Provide EXACTLY two prompts, in the exact format shown below. MUST 30-50 words each.
- The two prompts must be **extremely similar**: over 90% of words should be identical to ensure scene continuity. Only the weather-related descriptions should differ significantly.
- Make weather changes between the prompts **obvious and dramatic** visually, but keep all other scene elements identical (terrain layout, camera angle, background features).
- Keep prompts DETAILED with specific weather and terrain descriptions.
- The BEFORE prompt must clearly describe the original weather condition.
- The AFTER prompt must clearly describe the target weather condition.
- Include specific details: visibility, precipitation type, sky condition, lighting, terrain wetness/dryness, atmospheric effects.

FORMAT (must match exactly):
INPUT PROMPT: Generate image: [detailed description of BEFORE weather state in off-road scenario]
OUTPUT PROMPT: Generate image: [detailed description of AFTER weather state in off-road scenario]
...
def generate_prompts_with_vl(vl_model, vl_processor, edit_instruction, device):
    """Use Qwen2.5-VL to generate two detailed prompts for weather edits"""
    try:
        messages = [{
            "role": "user",
            "content": [
        ...
        return (
            f"Generate image: Off-road terrain scene, original weather condition, {edit_instruction}",
            f"Generate image: Off-road terrain scene, changed weather condition, {edit_instruction}"
        )
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # Generate input image (before weather change)
                with torch.no_grad():
                ...
                # Generate output image (after weather change)
                with torch.no_grad():
...
def save_checkpoint_hf_dataset(data_list, checkpoint_num):
    ...
    try:
        ...
        checkpoint_file = f"./checkpoint{checkpoint_num}_weather.parquet"
...
def main():
    ...
    # Read instructions from weather.txt
    weather_file = 'Path_to_TerraBagel/data_generation/create_instruction/edit_instruction/weather.txt'
    with open(weather_file, 'r', encoding='utf-8') as f:
        instructions = [line.strip() for line in f.readlines() if line.strip()]
    
    print(f"Loaded {len(instructions)} weather edit instructions")
    ...
    if data_list:
        try:
            ...
            # Save as parquet
            dataset.to_parquet("./weather.parquet")
            print(f"\nFinal weather.parquet saved: {len(data_list)} samples")
            
            # Also save as HuggingFace dataset directory for easier loading
            dataset.save_to_disk("./weather_dataset")
            print(f"Dataset also saved to ./weather_dataset directory")
