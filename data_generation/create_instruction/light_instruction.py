...
def generate_instructions(gpu_id, num_batches, output_queue, model_path, batch_size=100):
    ...
    try:
        ...
        for batch_idx in range(num_batches):
            # 修改后的prompt：要求生成10个光线变化指令
            prompt = """Generate EXACT 10 simple and clear image edit instructions for off-road robot scene lighting transformations. Each instruction should be on a separate line.
...
- Should be terrain scenes. Avoid urban streets, indoor environments......
- Transform lighting conditions with specific causes and effects
- Be concise and direct (10-40 words maximum per instruction)
- Use simple language
- MUST be in English only
- Each instruction MUST describe DIFFERENT lighting transformations
- Each instruction MUST specify the cause of lighting change (time of day, weather events, natural phenomena)
- Each instruction MUST use DIFFERENT terrain types as starting points

Lighting transformation types to use (use variety):
- Time transitions: bright midday → golden hour sunset, daytime → dusk with fading light, afternoon → night with moonlight
- Dawn/sunrise: pre-dawn darkness → early morning light, night → sunrise with warm glow, dark → dawn with soft illumination
- Artificial lighting: daytime → night with vehicle headlights, natural light → dim path with scattered lanterns, bright → dark with spotlight beams
- Shadow changes: harsh noon shadows → soft overcast lighting, bright sun → filtered forest canopy light, direct light → dappled shade
- Low light conditions: bright day → twilight with reduced visibility, clear → foggy dim atmosphere, well-lit → moonlit scene
- Extreme lighting: normal → glare from low sun, balanced → high contrast with strong backlight, moderate → dramatic side lighting

Good examples:
"Transform this bright midday desert trail with harsh shadows into a golden hour scene with warm orange light and long soft shadows."
"Convert this daytime forest path with dappled sunlight into a twilight setting with dim bluish light and reduced visibility."
"Change this afternoon rocky trail under clear sky into a night scene illuminated only by vehicle headlights creating focused beams."
"Alter this overcast morning grassland with soft even light into a sunrise scene with warm pink and orange hues breaking through."
"Edit this sunny dirt road with strong overhead light into a dusk environment with fading purple-blue light and emerging shadows."
...
Now generate 10 instructions in this style. Output ONLY the instructions, one per line, without numbering, quotes, or any other text:"""
            ...
            with torch.no_grad():
                generated_ids = model.generate(
                    **inputs,
                    max_new_tokens=1000,  # 增加以容纳更长的光线描述
                    temperature=0.85,
                    top_p=0.9,
                    do_sample=True,
            ...
            for line in lines:
                ...
                if word_count < 5 or word_count > 70:  # 允许稍长以容纳光线描述
                    continue
                
                # 跳过明显不是指令的行（如包含"example", "instruction"等元文本）
                lower_inst = instruction.lower()
                if any(skip_word in lower_inst for skip_word in ['example:', 'instruction:', 'note:', 'format:', 'requirements:']):
                    continue
...
def main():
    ...
    num_gpus = 8
    total_instructions = 10000  # 总目标数量改为10000
    # 由于每次生成10个，所以需要的batch数量是 10000 / 10 / 8
    instructions_per_gpu = total_instructions // num_gpus  # 每个GPU生成1250个
    num_batches_per_gpu = instructions_per_gpu // 10  # 每个GPU需要125次生成（每次10个）
    batch_size = 100  # 每100条保存一次
    
    # 创建输出目录
    output_dir = Path("edit_instruction")
    output_dir.mkdir(exist_ok=True)
    output_file = output_dir / "light.txt"  # 修改文件名为lighting_change
    ...
    print(f"Starting generation of {total_instructions} lighting transformation instructions across {num_gpus} GPUs")
