...
def generate_instructions(gpu_id, num_batches, output_queue, model_path, batch_size=100):
    ...
    try:
        ...
        for batch_idx in range(num_batches):
            # 修改后的prompt：要求生成10个场景变化指令
            prompt = """Generate EXACT 10 simple and clear image edit instructions for off-road robot scene transformations. Each instruction should be on a separate line.
...
- Transform terrain/surface characteristics (e.g., vegetation density, surface condition, texture, obstacles)
- Be concise and direct (10-40 words maximum per instruction)
- Use simple language
- MUST be in English only
- Each instruction MUST describe DIFFERENT types of terrain transformations
- Each instruction MUST use DIFFERENT terrain types as starting points

Terrain transformation types to use (use variety):
- Vegetation changes: dense grass → sparse grass, clear path → overgrown trail, bare ground → vegetation-covered
- Surface condition: dry road → muddy road, smooth pavement → cracked pavement, solid ground → waterlogged terrain
- Texture changes: fine sand → coarse gravel, smooth dirt → rocky surface, flat terrain → rutted path
- Obstacle changes: clear trail → debris-covered path, open area → boulder-strewn terrain, smooth surface → root-covered ground
- Wear/degradation: maintained road → eroded road, intact surface → damaged surface, fresh trail → weathered trail

Good examples:
"Transform this dense grassland with thick vegetation into a sparse grassland with patches of bare soil and scattered grass tufts."
"Convert this dry dirt road with smooth surface into a muddy rutted path with deep tire tracks and standing water puddles."
"Change this clear forest trail with compact soil into an overgrown path with exposed tree roots and fallen branches."
"Alter this fine sandy desert terrain into a coarse gravel surface with scattered rocks and rough texture."
...
def main():
    ...
    output_file = output_dir / "scene.txt"  # 修改文件名为scene_change
    ...
    print(f"Starting generation of {total_instructions} scene transformation instructions across {num_gpus} GPUs")
